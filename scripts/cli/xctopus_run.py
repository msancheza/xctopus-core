...
# Add parent directory to path to import xctopus
...
try:
    ...
    print("[TIP] Make sure you're running from the xctopus directory")
    sys.exit(1)
...
def main():
    ...
    try:
        # Create pipeline
        if config_path:
            pipeline = XctopusPipeline(dataset_path, config=config_path)
        else:
            pipeline = XctopusPipeline(dataset_path)
        ...
        # Save checkpoint if output_dir specified
        if args.output_dir:

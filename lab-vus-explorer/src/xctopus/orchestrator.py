...
from .settings import (
    ...
    SMS_DEFAULT_THRESHOLD,
    get_adaptive_lora_rank,
...
class Orchestrator:
    ...
    def __init__(
        ...
        logger.debug(f"DataManager initialized with {len(dataset_paths or {})} dataset paths and {len(text_columns or [])} custom columns")
    ...
    def _find_similar_buffer(self, embedding: torch.Tensor) -> Optional[str]:
        ...
        # [ANÁLISIS CRÍTICO]: Adaptabilidad a Dos Modelos (384d vs 768d).
        # El ruteo vectorizado debe manejar fallos de dimensión si la DB contiene
        # residuos de un modelo anterior. 
        try:
            # embedding: [Dynamic] -> [1, Dynamic]
            # centroids: [N, Dynamic]
            if centroids.shape[1] != embedding.shape[0]:
                logger.warning(
                    f"Dimension mismatch in vector routing: "
                    f"DB centroids={centroids.shape[1]}, current embedding={embedding.shape[0]}. "
                    f"Ignoring existing buffers."
                )
                return None
            ...
            # [BLINDAJE]: Protección contra estados corruptos de la DB
            logger.error(f"Critical failure in vectorized similarity check: {e}")
            return None
    ...
    def process_embedding(
        ...
        # 0. Extract Gene Identity signal for Veto Rule
        target_gene = self._extract_gene(text) if text else None

        # 1. Routing
        node_id, score, _ = self.filter.route(embedding, target_gene=target_gene)
        ...
        try:
            ...
            # [v7.4 Fix]: Inject Truth Signal (Gene Identity)
            node_label = self.repository.get_node_label(node_id)
            gene_identity = self._translate_legacy_identity(node_label)
            
            # [Phase 8] Knowledge Injection
            knowledge_summary = self.repository.get_knowledge_summary(node_id)
            
            # Run Transformer (Singleton + Safe Adapter)
            # We pass the gene_identity to be injected into the system prompt if needed
            output = self.transformer.forward_with_node(
                embedding, 
                lora_weights, 
    ...
    def process_batch(
        ...
        # 0. Extract Target Genes for Veto Rule
        target_genes = [self._extract_gene(t) for t in texts] if texts else None

        # 1. Vectorized Routing
        # returns list of (node_id, score)
        routing_decisions = self.filter.route_batch(embeddings, target_genes=target_genes)
        ...
        for node_id, indices in node_groups.items():
            ...
            try:
                ...
                for i, idx in enumerate(indices):
                    src_id = group_source_ids[i] if i < len(group_source_ids) else None
                    txt = texts[idx] if texts else None
                    final_results[idx] = self._handle_new_buffer(group_embeddings[i], source_id=src_id)
                    
        return final_results

    def _extract_gene(self, text: str) -> Optional[str]:
        """
        Extract gene identity signal from text.
        Pattern: [GENE_IDENTITY: XXX]
        """
        import re
        match = re.search(r'\[GENE_IDENTITY:\s*([^\]]+)\]', text)
        if match:
            return match.group(1).strip()
        return None

    def _translate_legacy_identity(self, node_label: str) -> Optional[str]:
        """
        [Legacy Wrapper] Extracts Gene Identity from v72/v73 node labels.
        Example: 'BRCA1_INTRONIC_RIGOR_COIN_v72...' -> 'BRCA1'
        """
        if not node_label: return None
        node_label = node_label.upper()
        
        if "BRCA1" in node_label: return "BRCA1"
        if "BRCA2" in node_label: return "BRCA2"
        if "TP53" in node_label: return "TP53"
        if "PEX14" in node_label: return "PEX14"
    ...
    def process_decision(
        ...
        # [ANÁLISIS CRÍTICO]: Scan técnico de tipos y formas.
        # Validamos que el tensor sea 1D, pero dejamos que la dimensión real
        # se valide contra el nodo destino específico en process_decision.
        if embedding.dim() > 1:
        ...
        if node_id_or_buffer == "NEW_BUFFER":
            ...
            if buffer_size >= BUFFER_THRESHOLD:
                ...
                return new_node_id if new_node_id else buffer_id
            else:
    ...
    def promote_buffer_to_kn(self, buffer_id: str) -> str:
        ...
        # ========================================================================
        # [GOLD] MITOSIS CHECK (Fase 4: Cell Division)
        # ========================================================================
        # Check if this buffer is a "Tumor" (High Mass + High Variance)
        # and needs to be split before becoming a single node.
        # ========================================================================
        try:
            mitosis = MitosisEngine(self.repository, self.filter)
            # Check eligibility (Mass >= 50, Variance > 0.35)
            if mitosis.check_mitosis_eligibility(buffer_id):
                children = mitosis.execute_mitosis(buffer_id)
                # If mitosis actually happened (returned > 1 children)
                if len(children) > 1:
                    logger.info(f"🧬 Mitosis successful! Buffer {buffer_id} split into {children}")
                    # Return the first child ID so the caller has a valid node reference
                    # The other child is also a valid KN now.
                    return children[0]
                # If returned original buffer_id, continue with normal promotion
        except Exception as e:
            logger.error(f"Mitosis execution failed for {buffer_id}: {e}", exc_info=True)
        ...
        # ========================================================================
        # [GOLD]: Maturity Gate (Fase 7)
        # ========================================================================
        # Rationale: Only promote buffers that reach a structural maturity 
        # threshold (SMS). Immature nodes created prematurely cause PPL drift.
        # ========================================================================
        real_variance = initial_m2 / (initial_mass - 1) if initial_mass > 1 else 0.0
        sms = self.filter.calculate_sms_scalar(initial_mass, real_variance)
        
        if sms < SMS_DEFAULT_THRESHOLD:
            logger.info(
                f"Buffer {buffer_id} reached volume threshold ({initial_mass}), but failed "
                f"Maturity Gate: SMS={sms:.3f} < threshold={SMS_DEFAULT_THRESHOLD}. "
        ...
        # Calculate Adaptive LoRA Rank (Fase 2 X-DETOX)
        lora_rank = get_adaptive_lora_rank(initial_mass)
        logger.info(f"Assigned adaptive LoRA rank={lora_rank} for node '{node_id}' (mass={initial_mass})")
        ...
        kn = KnowledgeNode(
            ...
            optimal_training_threshold=SMS_DEFAULT_THRESHOLD,
            mental_age_score=sms,
            lora_rank=lora_rank
        )
        ...
        self.repository.save_new_kn(
            ...
            optimal_training_threshold=signature.get("optimal_training_threshold", SMS_DEFAULT_THRESHOLD),
            mental_age_score=signature.get("mental_age_score", 0.0),
            lora_rank=signature.get("lora_rank"),
            gene=None # Organic clusters don't have a fixed gene identity by default
        )
    ...
    def _calculate_affinity(
        ...
        """
        [GOLD]: Calculate affinity for Inheritance (Fix #2: Sweet Spot).
        
        FIX PPL: Herencia solo en Sweet Spot 0.65-0.82
        Too similar (>0.85) = copia inútil
        Too different (<0.65) = transferencia pobre
        """
        titan_sig = self.repository.get_signature(titan_id)
        if not titan_sig or not self.repository.is_trained(titan_id):
            return None
            
        # Verify not FAILED
        try:
            if self.repository.get_peft_training_status(titan_id) == 'FAILED':
                return None
        except Exception:
            return None
        ...
        child_centroid_tensor = child_centroid.to(device=DEVICE, dtype=DTYPE)
        titan_centroid_tensor = titan_sig['centroid'].to(device=DEVICE, dtype=DTYPE)
        
        similarity = F.cosine_similarity(
            child_centroid_tensor.unsqueeze(0), 
            titan_centroid_tensor.unsqueeze(0)
        ).item()
        
        # ✅ FIX: Sweet Spot Herencia (Fix #2)
        # range: 0.65 - 0.82
        threshold = 0.65 if use_progressive_threshold else 0.70
        SWEET_SPOT_MAX = 0.82  # ✅ NUEVO: Límite superior
        
        if threshold <= similarity <= SWEET_SPOT_MAX:
            logger.debug(f"Herencia Sweet Spot: {titan_id}→child (sim={similarity:.3f})")
            return titan_id, similarity
        
        logger.debug(
            f"Herencia rechazada: sim={similarity:.3f} "
            f"(sweet spot: {threshold}-{SWEET_SPOT_MAX})"
        )
        return None
    ...
    def run_deferred_training(self, progress_interval: int = 1) -> Dict[str, Any]:
        ...
        # ========================================================================
        # Phase 0.2: Recoverable FAILED Nodes (2026-01-31) - Anti-Stalling
        # ========================================================================
        # Identify nodes in FAILED state that have gained mass and could be retried.
        # This automatically handles nodes that were skipped due to lack of texts.
        # ========================================================================
        recoverable = self.repository.get_recoverable_failed_nodes(TRAINING_THRESHOLD)
        if recoverable:
            logger.info(f"🔄 Found {len(recoverable)} recoverable FAILED nodes. Marking for retry.")
            for node_id in recoverable:
                try:
                    self.repository.mark_for_training(node_id, is_retraining=False)
                except Exception as e:
                    logger.debug(f"Failed to remark node {node_id} for training: {e}")
        ...
        if trained > 0:
            ...
            try:
                ...
                results_summary = ephemeral_evaluator.evaluate_all_adapters(
                    node_ids=ordered_pending,
                    progress_interval=progress_interval
                )
                
                # [GOLD]: Mental Age Audit (Fase 9)
                # Convert list results to dict for validation {node_id: metrics}
                # Filter only successful evaluations
                node_metrics = {}
                for res in results_summary.get('results', []):
                    if res.get('status') == 'success' and 'metrics' in res:
                        node_metrics[res['node_id']] = res['metrics']

                # Process results: split if confused (high PPL), prune if garbage
                self.validate_node_maturity(node_metrics)
                            ...
                            if signature_check_retry:
                                ...
                                try:
                                    self.repository.save_new_kn(
                                        ...
                                        optimal_training_threshold=current_sig.get("optimal_training_threshold", 0.0),
                                        mental_age_score=current_sig.get("mental_age_score", 0.0),
                                        lora_rank=current_sig.get("lora_rank")
                                    )
    ...
    def _check_and_trigger_training(self, node_id: str, signature: Dict[str, Any]) -> None:
        """
        [GOLD]: Training Gate Reforzado (Fix #1).
        Triple validación: volumen + coherencia + madurez.
        FIX PPL: Rechaza nodos inmaduros que causan drift.
        """
        try:
            # Validate signature
            ...
            variance = signature.get("variance", 1.0)
            
            # 🛡️ NUEVO: Triple Gate (Fix #1)
            # volume: mass >= TRAINING_THRESHOLD (12)
            # coherence: variance <= 0.3
            # maturity: SMS >= 1.8
            sms_score = self.filter.calculate_sms_scalar(mass, variance)
            
            training_conditions = {
                "volume": mass >= TRAINING_THRESHOLD,
                "coherence": variance <= 0.3,
                "maturity": sms_score >= 1.8
            }
            
            should_train = all(training_conditions.values())
            
            # Log detallado para debug PPL
            logger.debug(
                f"Training Gate {node_id}: mass={mass}, var={variance:.3f}, "
                f"SMS={sms_score:.3f}, conditions={training_conditions}, decision={should_train}"
            )
            ...
            if not self.repository.is_trained(node_id):
                try:
                    ...
                    logger.info(f"✅ Training APPROVED {node_id}: {training_conditions}")
                except Exception:
                    pass
                return
            
            # ========================================================================
            # Case 2: Re-training (Training Delta)
            # ========================================================================
            if TRAINING_DELTA_MULTIPLIER > 0:
                ...
                if last_training_mass > 0 and mass >= last_training_mass * TRAINING_DELTA_MULTIPLIER:
                    try:
                        ...
                        with self._pending_cache_lock:
                            self._pending_training_cache.add(node_id)
                        logger.info(f"🔄 Re-training APPROVED {node_id}")
                    except Exception:
                        pass
                    return
    ...
    def _async_train_kn(self, node_id: str, epoch_callback: Optional[Callable[[int, int], None]] = None) -> None:
        ...
        try:
            ...
            raw_texts = self.data_manager.get_texts_from_pointers(source_ids)
            
            # Phase 4: Atomic Injection - Prepend System Prompt
            texts = []
            if raw_texts:
                for text in raw_texts:
                    if text and "[ATOMIC_DATA]" in text:
                        # Prepend "Strict Teacher" prompt to Atomic Data
                        texts.append(f"{ATOMIC_SYSTEM_PROMPT}\n\n{text}")
                    else:
                        texts.append(text)
            
            # ========================================================================
            # Phase 2: Replay Buffer Injection (2025-02-07)
            # ========================================================================
            # Retrieve atomic representatives to prevent catastrophic forgetting
            # These are "best-of" samples stored during ingestion
            # ========================================================================
            try:
                representatives = thread_repo.get_node_representatives(node_id)
                if representatives:
                    logger.info(f"💉 Injecting {len(representatives)} atomic representatives for node '{node_id}'")
                    # Append representatives to training set
                    for rep_text in representatives:
                         if rep_text and "[ATOMIC_DATA]" in rep_text:
                            texts.append(f"{ATOMIC_SYSTEM_PROMPT}\n\n{rep_text}")
                         else:
                            texts.append(rep_text)
            except Exception as e:
                logger.warning(f"Failed to inject representatives for {node_id}: {e}")
            ...
            with self._training_lock:
                training_result = self.transformer.train_kn_adapter(
                    ...
                    mental_age_score=current_sig.get("mental_age_score"),  # Phase 8: Adaptive Maturity Tuning
                    lora_rank=current_sig.get("lora_rank")  # Fase 2 X-DETOX: Adaptive Rank
                )
            ...
            if training_result is None:
                ...
                if training_result is None:
                    ...
                    # Phase 2: Distinguish between insufficient data (protection) and real errors
                    if len(texts) < MIN_TRAINING_TEXTS:
                        logger.warning(
                            f"⚠️ Training skipped for node '{node_id}' due to insufficient data "
                            f"({len(texts)} < {MIN_TRAINING_TEXTS}). Marking as FAILED. "
                            f"Will retry automatically once node mass grows."
                        )
                    else:
                        logger.error(f"❌ Training failed for node '{node_id}' (execution error). Marking as FAILED.")
    ...
    def validate_node_maturity(self, eval_results: Dict[str, Dict[str, Any]]) -> None:
        """
        [GOLD]: Mental Age Audit (MAA).
        Analiza los resultados de PPL/Loss y decide si el nodo necesita cirugía (Split/Prune).
        
        Added: Fase 9 (Mental Age Governance)
        Rationale: High PPL indicates a node has become too generalized or "confused".
        """
        from .settings import (
            PPL_MAX_THRESHOLD, 
            PPL_SPLIT_THRESHOLD, 
        ...
        if not eval_results:
            logger.debug("Mental Age Audit: No results to analyze.")
            return

        logger.info(f"🧠 Mental Age Audit: Analizando {len(eval_results)} nodos...")
        
        for node_id, metrics in eval_results.items():
            ppl = metrics.get('perplexity', 0.0)
            if ppl == 0.0: continue # Skip if no PPL recorded
            
            # 1. Poda del adaptador (Pruning)
            # Si PPL es catastrófica (>15.0), el adaptador no es confiable.
            if ppl > PPL_MAX_THRESHOLD:
                logger.warning(
                    f"💀 PPL Crítica ({ppl:.2f}) en {node_id}. Marcando como FAILED para reintento."
                )
                try:
                    self.repository.set_peft_training_status(node_id, 'FAILED')
                except Exception as e:
                    logger.error(f"Error marking node {node_id} as FAILED: {e}")
                continue
            
            # 2. Fragmentación (Split)
            # Si PPL es alta (>25.0) y tiene masa suficiente, dividirlo.
            if ppl > PPL_SPLIT_THRESHOLD:
                try:
                    sig = self.repository.get_signature(node_id)
                    mass = sig.get('mass', 0) if sig else 0
                    
                    if mass >= MIN_TEXTS_FOR_SPLIT:
                        logger.info(f"✂️ Nodo CONFUSO detectado ({ppl:.2f}). Iniciando SPLIT en {node_id}.")
                        self._trigger_node_split(node_id)
                    else:
                        logger.debug(f"Nodo confuso {node_id} pero masa insuficiente ({mass}) para split.")
                        
                    # [GOLD] Actualización Dinámica de Thresholds
                    # Si sobrevivió al split pero tiene PPL alta, aumentar su threshold de rigurosidad
                    # para el siguiente ciclo.
                    current_sms = sig.get('mental_age_score', 0.0)
                    new_threshold = 4.0 # Base
                    if ppl > 10.0:
                        new_threshold = 5.0 # Más estricto
                    if ppl > 20.0:
                        new_threshold = 6.0 # Muy estricto
                        
                    self.repository.update_node_maturity(node_id, current_sms, new_threshold)

                except Exception as e:
                    logger.error(f"Error during split analysis for {node_id}: {e}")

    def _trigger_node_split(self, node_id: str) -> bool:
        """
        [GOLD]: Mecanismo de re-clusterización para nodos obesos o confusos.
        Divide un nodo en 2 nuevos sub-nodos más precisos usando KMeans.
        """
        try:
            from uuid import uuid4
            # 1. Recuperar todos los embeddings del nodo
            embeddings = self.repository.get_node_embeddings(node_id)
            if len(embeddings) < 4:
                logger.debug(f"Split abortado para {node_id}: insuficientes embeddings ({len(embeddings)})")
                return False
                
            # Convertir a numpy para KMeans
            data = torch.stack(embeddings).cpu().numpy()
            
            # 2. Re-clusterizar (K=2)
            kmeans = KMeans(n_clusters=2, n_init='auto', random_state=42)
            labels = kmeans.fit_predict(data)
            
            # 3. Crear dos nuevos buffers temporales
            new_nodes = []
            for i in range(2):
                cluster_embeddings = [embeddings[j] for j, label in enumerate(labels) if label == i]
                if cluster_embeddings:
                    # Crear nuevo identificador de buffer
                    buffer_id = f"split_{node_id[:8]}_{i}_{str(uuid4())[:4]}"
                    
                    self.repository.create_buffer(buffer_id)
                    for emb in cluster_embeddings:
                        self.repository.add_embedding_to_buffer(buffer_id, emb)
                    
                    # Promover inmediatamente
                    try:
                        new_node_id = self.promote_buffer_to_kn(buffer_id)
                        new_nodes.append(new_node_id)
                    except Exception as prom_err:
                        logger.error(f"Error promoviendo buffer de split {buffer_id}: {prom_err}")
            
            if len(new_nodes) >= 2:
                logger.info(f"✅ Split exitoso: {node_id} -> {new_nodes}")
                # 4. Eliminar el nodo original "obeso"
                self.repository.delete_node(node_id)
                return True
            else:
                logger.warning(f"Split fallido para {node_id}: Se generaron {len(new_nodes)} nodos.")
                return False
                
        except Exception as e:
            logger.error(f"Error en _trigger_node_split para {node_id}: {e}", exc_info=True)
            return False
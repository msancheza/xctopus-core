...
def _calculate_training_epochs(
    ...
    Rigor por Densidad [GOLD] (Fase 7):
    Epochs = clip(log1p(mass / (1 + variance)) * 1.5, 1, 7)
    Rationale: Rigor proportional to information density. High density (mass/coherence) 
    deserves higher rigor. Volume discount is removed to favor expertise over blind 
    volume limits.
    ...
    # [GOLD]: Entrenamiento por Densidad de Información.
    # Rigor proportional to density: mass / (1 + variance)
    density = mass / (1 + variance)
    complexity = np.log1p(density)
    ...
    # ========================================================================
    # Map to epochs (factor 1.5 for density range, range 1-7)
    # CRITICAL: Safety Floor = 1 epoch (never 0)
    epochs = int(np.clip(complexity * 1.5, 1, 7))
    
    # [GOLD]: Volume Discount Removed. 
    # Expertise is now rewarded via density, no longer capped by blind volume limits.

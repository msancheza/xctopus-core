...
class DataManager:
    ...
    def __init__(
        ...
        # [CONTEXT ENGINE REMOVED]
        # ContextInjector logic removed as requested.
        ...
        try:
            ...
            for chunk_idx, chunk in enumerate(pd.read_csv(
                ...
                if text_column is None:
                    # Priority 1: User specified columns via CLI
                    priority_list = self.text_columns + ['formatted_text', 'text', 'summaries', 'titles', 'abstract', 'content', 'body']
                ...
                for idx, row in chunk.iterrows():
                    # Phase 4 Atomic Injection: Prioritize content_text if available
                    if 'content_text' in chunk.columns:
    ...
    def _load_csv_dataset(self, dataset_path: str, dataset_name: str) -> Dict[str, str]:
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                # [CONTEXT ENGINE] Enrich text if injector is active
                if self.context_injector:
                    try:
                        record = row.to_dict()
                        record['text'] = str(text).strip()
                        enriched = self.context_injector.enrich_record(record)
                        text = enriched['eda_text']
                    except Exception as e:

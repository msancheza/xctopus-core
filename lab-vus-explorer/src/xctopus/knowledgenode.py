...
class LocalFilter:
    ...
    def accepts(self, embedding: torch.Tensor, centroid: Optional[torch.Tensor]) -> bool:
        ...
        Args:
            embedding: Embedding tensor [Dynamic]
            centroid: Node centroid [Dynamic] or None if it's the first embedding
        ...
        # [ANÁLISIS CRÍTICO]: Adaptabilidad 384d/768d.
        # Validamos que las dimensiones coincidan antes de la operación de similitud.
        try:
            ...
            if embedding.shape != centroid.shape:
                logger.error(f"Dimension mismatch in LocalFilter: embedding={embedding.shape}, centroid={centroid.shape}")
                return False
            ...
            # [BLINDAJE]: Captura de fallos en flujo de tensores
            logger.error(f"Critical error in LocalFilter.accepts: {e}")
            return False
    ...
    def calculate_affinity(
        ...
        Args:
            embedding: Embedding tensor [Dynamic]
            centroid: Node centroid [Dynamic]
        ...
        """
        # [ANÁLISIS CRÍTICO]: Escaneo técnico de gestión de tensores.
        # Aseguramos robustez en la comparación de modelos con diferentes arquitecturas.
        try:
            ...
            # Ensure device and dtype
            embedding = embedding.to(device=DEVICE, dtype=DTYPE).squeeze()
            centroid = centroid.to(device=DEVICE, dtype=DTYPE).squeeze()

            if embedding.shape != centroid.shape:
                logger.warning(f"Affinity skipped due to dimension mismatch: emb={embedding.shape}, cent={centroid.shape}")
                return 0.0
            ...
            # [BLINDAJE]: Protección de flujos matemáticos
            logger.error(f"Calculus failure in calculate_affinity: {e}")
            return 0.0
...
class KnowledgeNode:
    ...
    def __init__(
        ...
        Args:
            ...
            optimal_training_threshold: [GOLD] SMS threshold (default: 0.0)
            mental_age_score: [GOLD] Evolution metric (default: 0.0)
        """
        # Unique identifier
        self.node_id = node_id or str(uuid4())
        
        # [GOLD]: Metadata de Madurez Estructural (Fase 7)
        self.optimal_training_threshold = optimal_training_threshold
        self.mental_age_score = mental_age_score
        self.lora_rank = lora_rank
        
        # Atomic Anchor properties (Fase 4)
        self.is_anchor = is_anchor
        self.rigor_inertia = rigor_inertia
    ...
    def process(self, embedding: torch.Tensor) -> bool:
        ...
        # [ANÁLISIS CRÍTICO]: Escaneo técnico de dimensiones
        # Eliminamos la dependencia rígida de EMBEDDING_DIM para permitir SapBERT.
        if self.centroid is not None:
            if embedding.shape[0] != self.centroid.shape[0]:
                raise ValueError(
                    f"Dimension mismatch in KnowledgeNode '{self.node_id}': "
                    f"expected {self.centroid.shape[0]}, received {embedding.shape[0]}"
                )
        ...
        # ========================================================================
        # LocalFilter: Membership Validation (Fix #3: Dynamic)
        # ========================================================================
        # Added: Fase 9 (Mental Age Governance)
        # Rationale: Mature nodes (low variance) must be stricter to protect PPL.
        # ========================================================================
        if self.mass > 0 and self.centroid is not None:
            # Calculate real variance for dynamic threshold
            real_variance = self.variance / (self.mass - 1) if self.mass > 1 else 0.0
            
            # Ensure device/dtype for calculation
            embedding = embedding.to(device=DEVICE, dtype=DTYPE)
            centroid = self.centroid.to(device=DEVICE, dtype=DTYPE)
            ...
            # [GOLD]: Dynamic s_local (Fix #3)
            # Normal range: S_MIN (0.60) -> S_MIN+0.05 (0.65)
            s_local = S_MIN + 0.05 * (1.0 - min(real_variance, 1.0))
            
            if similarity < s_local:
                logger.debug(
                    f"LocalFilter REJECT: node={self.node_id} | sim={similarity:.3f} < s_local={s_local:.3f} | "
                    f"mass={self.mass} | var={real_variance:.3f}"
                )
                return False
        
        # If mass is 0, we still use the standard LocalFilter check just in case
        # (though for mass=0 accepts() usually returns True)
        elif not self.local_filter.accepts(embedding, self.centroid):
            logger.debug(f"Embedding rejected by standard LocalFilter in node '{self.node_id}'")
            return False
        ...
        if self.centroid is None:
            ...
            # ========================================================================
            # Welford's Algorithm for Centroid (Updated for Inertia 2026-02-07)
            # ========================================================================
            # Formula: C_n = C_{n-1} + (x_n - C_{n-1}) / n
            # Inertia: Scale update by (1.0 - rigor_inertia)
            # If rigor_inertia = 1.0 (GOLD), centroid doesn't move.
            # ========================================================================
            ...
            # Apply Inertia: The effective learning rate is reduced
            learning_rate = (1.0 / self.mass) * (1.0 - self.rigor_inertia)
            self.centroid += delta * learning_rate
            
            # ========================================================================
            # Welford's Algorithm for Variance (M2)
            # ========================================================================
            # M2_n = M2_{n-1} + (x_n - C_{n-1}) · (x_n - C_n)
            # We calculate variance based on the actual movement of the centroid
            # ========================================================================
        ...
        # [X-DETOX] Update Adaptive LoRA Rank based on new mass
        self.lora_rank = get_adaptive_lora_rank(self.mass)
    ...
    def get_signature(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "optimal_training_threshold": self.optimal_training_threshold,
            "mental_age_score": self.mental_age_score,
    ...
    def from_repository(
        ...
        # ========================================================================
        # [GOLD]: Load maturity metadata (Fase 7)
        optimal_training_threshold = signature.get("optimal_training_threshold", 0.0)
        ...
        kn = cls(
            ...
            optimal_training_threshold=optimal_training_threshold,
            mental_age_score=mental_age_score,

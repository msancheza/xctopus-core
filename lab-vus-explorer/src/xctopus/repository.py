...
class KNRepository:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        self.conn.execute('PRAGMA foreign_keys = ON')  # [GOLD]: Integridad Referencial Innegociable.
        # [GOLD]: Integridad Innegociable. Set busy_timeout to handle concurrent writes gracefully (2025-01-14)
    ...
    def _create_tables(self) -> None:
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS nodes (
                ...
                rigor_inertia REAL DEFAULT 0.0,
                rigor_level TEXT,
                gene TEXT,
                functional_impact_summary TEXT,
        ...
        # [GOLD]: Madurez Estructural Metadatos (Fase 7)
        try:
            cursor.execute("ALTER TABLE nodes ADD COLUMN optimal_training_threshold REAL DEFAULT 0.0")
            cursor.execute("ALTER TABLE nodes ADD COLUMN mental_age_score REAL DEFAULT 0.0")
            logger.info("Schema Update: Added maturity metadata columns to nodes table")
        except sqlite3.OperationalError:
        ...
        # [GOLD]: Adaptive LoRA Rank (Fase 2 X-DETOX)
        try:
            cursor.execute('ALTER TABLE nodes ADD COLUMN lora_rank INTEGER')
            logger.info("Schema Update: Added 'lora_rank' column to nodes table")
        except sqlite3.OperationalError:
        ...
        try:
            cursor.execute("ALTER TABLE nodes ADD COLUMN gene TEXT")
            logger.info("Schema Update: Added 'gene' column to nodes table")
        except sqlite3.OperationalError:
            pass # Column likely exists

        # ========================================================================
        # Phase 4: Atomic Knowledge Nodes (2026-02-07)
        # ========================================================================
        try:
            cursor.execute("ALTER TABLE nodes ADD COLUMN is_anchor INTEGER DEFAULT 0")
            cursor.execute("ALTER TABLE nodes ADD COLUMN rigor_inertia REAL DEFAULT 0.0")
            cursor.execute("ALTER TABLE nodes ADD COLUMN rigor_level TEXT")
            logger.debug("Atomic KN columns (is_anchor, rigor_inertia, rigor_level) added (migration Phase 4)")
        except sqlite3.OperationalError:
            pass
        
        # ========================================================================
        # Phase 8: Knowledge Injection (2026-02-07)
        # ========================================================================
        try:
            cursor.execute("ALTER TABLE nodes ADD COLUMN functional_impact_summary TEXT")
            logger.debug("Schema Update: Added 'functional_impact_summary' column")
        except sqlite3.OperationalError:
            pass
        
        # ========================================================================
        # Phase 2: Replay Buffer Support (2026-02-07)
        # ========================================================================
        # Stores text representatives for atomic nodes to prevent catastrophic forgetting
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS node_representatives (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                node_id TEXT NOT NULL,
                text_content TEXT NOT NULL,
                rep_type TEXT DEFAULT 'ATOM_REP',
                created_timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY(node_id) REFERENCES nodes(node_id) ON DELETE CASCADE
            )
        ''')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_node_reps_node_id ON node_representatives(node_id)')
    ...
    def _blob_to_tensor(self, blob: bytes, shape: Optional[tuple] = None) -> torch.Tensor:
        """
        Convert a sqlite3 BLOB to a PyTorch tensor.
        
        Args:
            blob: BLOB bytes
            shape: Optional target shape. If None, detects automatically from blob size.
        
        Returns:
            Tensor on the DEVICE specified in settings
        """
        # [ANÁLISIS CRÍTICO]: Adaptabilidad a Dos Modelos.
        # Eliminamos la necesidad de EMBEDDING_DIM fijo al cargar de la DB.
        try:
            if blob is None:
            ...
            # [BLINDAJE]: Si el blob está vacío o corrupto, notificamos pero no fallamos.
            if array.size == 0:
                logger.warning("Empty blob received from database. Returning zeros.")
                return torch.zeros(1, device=DEVICE, dtype=DTYPE)

            if shape:
                # Si se pide una forma específica (ej: para batches o matrices), validamos.
                if np.prod(shape) != array.size:
                    logger.error(f"Shape mismatch: {shape} needs {np.prod(shape)} elements, but blob has {array.size}")
                    # Fallback to 1D
                    array = array.copy()
                else:
                    array = array.reshape(shape).copy()
            else:
                # Inferencia automática: devolvemos 1D con la longitud natural del vector (384 o 768).
                array = array.copy()
            ...
            logger.error(f"Failed to deserialize blob: {e}")
            return torch.zeros(1, device=DEVICE, dtype=DTYPE)
    ...
    def get_all_signatures(self) -> List[Dict[str, Any]]:
        ...
        try:
            cursor = self.conn.cursor()
            cursor.execute("SELECT node_id, label, centroid, mass, variance, parent_node_id, parent_similarity, inheritance_type, optimal_training_threshold, mental_age_score, lora_rank, is_anchor, rigor_inertia, rigor_level FROM nodes WHERE status = 'KN'")
            rows = cursor.fetchall()
            
            signatures = []
            for row in rows:
                centroid = self._blob_to_tensor(row["centroid"])
                ...
                signatures.append({
                    ...
                    "optimal_training_threshold": row["optimal_training_threshold"] if "optimal_training_threshold" in row.keys() else 0.0,
                    "mental_age_score": row["mental_age_score"] if "mental_age_score" in row.keys() else 0.0,
                    "lora_rank": row["lora_rank"] if "lora_rank" in row.keys() else None,
                    # Atomic KN fields
                    "is_anchor": bool(row["is_anchor"]) if "is_anchor" in row.keys() else False,
                    "rigor_inertia": row["rigor_inertia"] if "rigor_inertia" in row.keys() else 0.0,
                    "rigor_level": row["rigor_level"] if "rigor_level" in row.keys() else None
                })
    ...
    def get_signature(self, node_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            cursor = self.conn.cursor()
            cursor.execute(
                "SELECT node_id, label, centroid, mass, variance, parent_node_id, parent_similarity, inheritance_type, optimal_training_threshold, mental_age_score, lora_rank FROM nodes WHERE node_id = ?",
                (node_id,)
            )
            row = cursor.fetchone()
    ...
    def get_knowledge_summary(self, node_id: str) -> Optional[str]:
        """
        Retrieve the functional impact summary for a node.
        ...
        """
        cursor = self.conn.cursor()
        cursor.execute("SELECT functional_impact_summary FROM nodes WHERE node_id = ?", (node_id,))
        row = cursor.fetchone()
        return row["functional_impact_summary"] if row else None

    def update_knowledge_summary(self, node_id: str, summary: str) -> None:
        """
        Update the functional impact summary for an existing node.
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "UPDATE nodes SET functional_impact_summary = ?, last_updated = CURRENT_TIMESTAMP WHERE node_id = ?",
            (summary, node_id)
        )
        self.conn.commit()
    ...
    def save_new_kn(
        ...
        optimal_training_threshold: float = 0.0,
        mental_age_score: float = 0.0,
        lora_rank: Optional[int] = None,
        is_anchor: bool = False,
        rigor_inertia: float = 0.0,
        ...
        try:
            cursor = self.conn.cursor()
            cursor.execute(
                "INSERT INTO nodes (node_id, label, gene, functional_impact_summary, centroid, mass, variance, status, parent_node_id, parent_similarity, inheritance_type, origin_type, optimal_training_threshold, mental_age_score, lora_rank, is_anchor, rigor_inertia, rigor_level) VALUES (?, ?, ?, ?, ?, ?, ?, 'KN', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (node_id, label, gene, functional_impact_summary, self._tensor_to_blob(centroid), mass, variance, parent_node_id, parent_similarity, inheritance_type, origin_type, optimal_training_threshold, mental_age_score, lora_rank, 1 if is_anchor else 0, rigor_inertia, rigor_level)
            )
            self._maybe_commit()
            logger.info(f"New KN created: {node_id} (label={label}, mass={mass}, variance={variance:.4f}, type={origin_type}, anchor={is_anchor})")
    ...
    def save_node_representatives(self, node_id: str, representatives: List[str], rep_type: str = 'ATOM_REP') -> None:
        """
        Save text representatives for a node (Replay Buffer).
        
        Args:
            node_id: Node ID
            representatives: List of text strings
            rep_type: Type of representative (default: 'ATOM_REP')
        """
        if not representatives:
            return
            
        cursor = self.conn.cursor()
        data = [(node_id, text, rep_type) for text in representatives]
        
        try:
            cursor.executemany(
                "INSERT INTO node_representatives (node_id, text_content, rep_type) VALUES (?, ?, ?)",
                data
            )
            self._maybe_commit()
            logger.debug(f"Saved {len(representatives)} representatives for node {node_id}")
        except Exception as e:
            logger.error(f"Failed to save representatives for {node_id}: {e}")

    def get_node_representatives(self, node_id: str, limit: Optional[int] = None) -> List[str]:
        """
        Retrieve text representatives for a node.
        
        Args:
            node_id: Node ID
            limit: Optional limit on number of representatives
            
        Returns:
            List of text strings
        """
        cursor = self.conn.cursor()
        query = "SELECT text_content FROM node_representatives WHERE node_id = ?"
        params = [node_id]
    ...
    def update_node_stats(self, node_id: str, centroid: torch.Tensor, mass: int, variance: float) -> None:
        ...
        try:
            # [GOLD] Recalculate SMS (Structure Maturity Score) on every stats update
            # Formula: SMS = wc * (1 - Variance) + wm * log1p(Mass)
            coherence = 1.0 - variance
            mass_log = np.log1p(mass)
            sms = (SMS_WEIGHT_COHERENCE * coherence) + (SMS_WEIGHT_MASS * mass_log)

            # [X-DETOX] Recalculate Adaptive LoRA Rank
            lora_rank = get_adaptive_lora_rank(mass)

            cursor = self.conn.cursor()
            cursor.execute(
                "UPDATE nodes SET centroid = ?, mass = ?, variance = ?, mental_age_score = ?, lora_rank = ?, last_updated = CURRENT_TIMESTAMP WHERE node_id = ?",
                (self._tensor_to_blob(centroid), mass, variance, float(sms), lora_rank, node_id)
            )
    ...
    def get_recoverable_failed_nodes(self, mass_threshold: int = 12) -> List[str]:
        """
        Get node IDs that are in FAILED state but have enough mass to retry.
        
        Added: 2026-01-31 (Anti-Stalling)
        
        Args:
            mass_threshold: Minimum mass to consider for retry
        ...
        try:
            ...
            cursor.execute("""
                SELECT node_id FROM nodes 
                WHERE peft_training_status = 'FAILED' 
                AND mass >= ?
                AND needs_training = 0
                ORDER BY mass DESC
            """, (mass_threshold,))
            rows = cursor.fetchall()
            return [row["node_id"] for row in rows]
            ...
            try:
                ...
                if cursor:
                    try:
                        ...
                        pass
                logger.error(f"Unexpected error updating training metadata {node_id}: {e}")
                raise

    def update_node_maturity(self, node_id: str, sms: float, threshold: float) -> None:
        """
        [GOLD] Update maturity metrics independently (Fase 9).
        Allows the Orchestrator to save dynamic threshold adjustments derived from PPL.
        """
        try:
            cursor = self.conn.cursor()
            cursor.execute(
                "UPDATE nodes SET mental_age_score = ?, optimal_training_threshold = ? WHERE node_id = ?",
                (sms, threshold, node_id)
            )
            self._maybe_commit()
            logger.debug(f"Maturity updated for {node_id}: SMS={sms:.2f}, Threshold={threshold:.2f}")
        except Exception as e:
            logger.error(f"Error updating maturity for {node_id}: {e}")
    ...
    def delete_node(self, node_id: str) -> bool:
        """
        [GOLD]: Delete a node from the repository.
        Cascades to node_memory, node_data_mapping, evaluation_metrics, etc.
        """
        if not node_id:
            return False
        ...
        try:
            ...
            try:
                self._lora_cache.cache_clear()
            except:
                pass
                
            logger.info(f"🗑️ Node {node_id} deleted from repository.")
            return True
        except Exception as e:
    ...
    def add_to_buffer(self, buffer_id: str, embedding: torch.Tensor, source_id: Optional[str] = None) -> None:
        ...
        try:
            ...
            if current_centroid_blob is None or current_size == 0:
                ...
                try:
                    current_centroid = self._blob_to_tensor(current_centroid_blob)
                    if current_centroid.shape != embedding.shape:
                        logger.warning(f"Dimension mismatch in buffer {buffer_id}. Resetting centroid to current embedding.")
                        new_centroid = embedding
                    else:
                        new_centroid = (current_centroid * current_size + embedding) / (current_size + 1)
                except Exception as e:
                    # [BLINDAJE]: Captura de fallos en actualización incremental
                    logger.error(f"Failed to update incremental centroid in buffer {buffer_id}: {e}")
                    new_centroid = embedding
    ...
    def add_embeddings_bulk(self, node_id: str, embeddings: List[torch.Tensor]) -> None:
        """
        [GOLD] Bulk add embeddings to memory. Crucial for Mitosis.
        """
        if not embeddings:
            return
        ...
        try:
            cursor = self.conn.cursor()
            data = [
                (node_id, self._tensor_to_blob(emb)) 
            ...
            self._maybe_commit()
            logger.debug(f"Bulk added {len(embeddings)} embeddings to {node_id}")
        except sqlite3.Error as e:
            logger.error(f"SQLite error bulk adding to {node_id}: {e}")
            raise
    ...
    def get_buffer_centroid(self, buffer_id: str) -> Optional[torch.Tensor]:
        ...
        if centroid_blob is not None:
            ...
            return self._blob_to_tensor(centroid_blob)
        else:
    ...
    def get_all_buffer_centroids(self) -> Tuple[List[str], torch.Tensor]:
        ...
        if not rows:
            # [ANÁLISIS CRÍTICO]: Fallback dinámico. 
            # Devolvemos un tensor vacío de dimensión 0x1 para que torch.stack no falle agresivamente
            # pero el receptor pueda determinar la dimensión real en el primer envío.
        ...
        for row in rows:
            ...
            if centroid_blob is not None:
                centroid = self._blob_to_tensor(centroid_blob)
    ...
    def get_all_sessions(self) -> List[Dict[str, str]]:
        """
        Retrieves all registered training sessions.
        
        Returns:
            List of dicts with 'session_id', 'dataset_name', and 'dataset_path'
        """
        try:
            cursor = self.conn.cursor()
            cursor.execute("SELECT session_id, dataset_name, dataset_path FROM training_sessions")
            rows = cursor.fetchall()
            return [
                {

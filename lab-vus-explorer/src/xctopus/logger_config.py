...
class RobustRotatingFileHandler(RotatingFileHandler):
    """
    Subclass of RotatingFileHandler that is more resilient to I/O and locking issues.
    If rotation (rename) fails due to file locks or I/O bottlenecks, it falls back
    to continuing to append to the current file instead of crashing the system.
    """
    def doRollover(self) -> None:
        """
        Perform a rollover, but catch OSErrors during file renaming.
        """
        try:
            # We try the standard rollover (renaming log -> log.1, etc.)
            super().doRollover()
        except OSError as e:
            # If rotation fails (common in cloud environments with multi-process locks)
            # we just keep writing to the same file. It's better than crashing.
            
            # CRITICAL: Re-open the stream if it was closed by super().doRollover()
            if not self.stream:
                try:
            ...
            # Low-level warning without triggering recursion
            sys.stderr.write(f"\n[Logging Warning] Rotation failed (file potentially locked): {e}. Continuing with current file.\n")
            
# Default values (can be overridden from settings)
_LOGGER_DIR = Path(__file__).resolve().parent
_DEFAULT_LOG_DIR = _LOGGER_DIR.parent.parent.parent / "logs"  # 3 levels to reach Studio Root/logs
        ...
        if '\n' in _stderr_buffer:
            ...
            for line in lines[:-1]:
                if line.strip():
                    ...
                    # Ultra-fast string check for common technical trace residues (low-level)
                    is_noise = (
                        "Traceback" in line or
                        'File "' in line or
                        "line " in line or
                        "Call stack" in line or
                        "Message:" in line or
                        "Arguments:" in line or
                        "OSError: [Errno 5]" in line or
                        "logging.handlers" in line or
                        "doRollover" in line or
                        "--- Logging error ---" in line
                    )
                    
                    if not is_tqdm_output and not is_hf_secret_warning and not is_noise and "[stderr]" not in line:
                        ...
                        # Even if already logged, still write to original stderr
                        pass
...
def _stderr_flush_interceptor(logger: logging.Logger):
    ...
    if _stderr_buffer.strip():
        ...
        # Ultra-fast string check for common technical trace residues
        is_noise = (
            "Traceback" in _stderr_buffer or
            'File "' in _stderr_buffer or
            "line " in _stderr_buffer or
            "Call stack" in _stderr_buffer or
            "Message:" in _stderr_buffer or
            "Arguments:" in _stderr_buffer or
            "OSError: [Errno 5]" in _stderr_buffer or
            "logging.handlers" in _stderr_buffer or
            "doRollover" in _stderr_buffer or
            "--- Logging error ---" in _stderr_buffer
        )
        
        if not is_tqdm_output and not is_hf_secret_warning and not is_noise and "[stderr]" not in _stderr_buffer:
            # Log to file ONLY (not console) to prevent recursion
            # Temporarily disable console handler, log, then re-enable
            file_logger = logging.getLogger("xctopus")
            console_handlers = []
...
def setup_logging(
    ...
    try:
        # Intentar usar RobustRotatingFileHandler con el nombre por defecto
        # Fixed: Custom handler that ignores rotation errors (locks) instead of crashing
